...
@Component({
  selector: 'app-planesmanagement',
  templateUrl: './planesmanagement.component.html',
  styleUrls: ['./planesmanagement.component.scss']
})
export class PlanesmanagementComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.i=0;
    this.adminservic.get_planmangement()
    .subscribe(data=>{
  ...
  create(){
    let dialogRef=this.dialog.open(PlanmanagementdialogComponent,{width:'25%'})
  ...
  delete(ref:any){
    console.log(ref);
  this.adminservic.delete_planmangement(ref._id)
  .subscribe(data=>{
    console.log(data);
    this.ngOnInit();
  },err=>{
  ...
  update(ref:any){
    console.log(ref);
    let dialogRef=this.dialog.open(UpdateplanmanagementComponent,{width:'25%',data:{
      id:ref._id,
      name:ref.name,
      cost:ref.cost,
